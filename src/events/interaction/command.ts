  ...
  async execute(interaction) {
    ...
    if (command.opt?.userPermissions) {
      ...
      if (missingUserPerms?.length) {
        await interaction.reply({
          content: `⚠️ You need the following permission${missingUserPerms.length > 1 ? 's' : ''}: ${missingUserPerms
            .map(x => inlineCode(x))
            .join(', ')}`,
          ephemeral: true,
        });
    ...
    if (command.opt?.botPermissions) {
      ...
      if (missingBotPerms?.length) {
        await interaction.reply({
          content: `⚠️ I need the following permission${missingBotPerms.length > 1 ? 's' : ''}: ${missingBotPerms
            .map(x => inlineCode(x))
            .join(', ')}`,
          ephemeral: true,
        });
