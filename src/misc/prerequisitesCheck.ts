...
async function checkEnvVariable(
  ...
  if (!process.env[variable]) {
    ...
    try {
      ...
      console.error(chalk.redBright(`Failed to set ${variable}.`));
      throw error;
    }
  }
}
...
function overwriteLine(filePath: string, variable: string, newValue: string): void {
  try {
    ...
    console.error(chalk.redBright(`Error writing to file ${filePath}.`));
    throw error;
  }
}
...
function createEnvFile(): void {
  if (!existsSync(".env")) {
    ...
    try {
      ...
      console.error(chalk.redBright("Failed to create .env file."));
      throw error;
    }
  } else {
...
async function askForGuildId(): Promise<void> {
  try {
    ...
    console.error(chalk.redBright("Failed to get response for global commands."));
    throw error;
  }
}
 ...
 */
export async function prerequisitesCheck(): Promise<void> {
